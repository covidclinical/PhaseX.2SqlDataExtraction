--##############################################################################
--##############################################################################
--### 4CE Phase 1.2 and 2.2 (Phase x.2)
--### Date: April 21, 2021 translated May 19, 2021
--### Database: Oracle
--### Data Model: i2b2
--### Created By: Griffin Weber (weber@hms.harvard.edu)
--### Translated to Oracle: Michele Morris ( mim18@pitt.edu)
--### Updated for U-M by Dom Silvio
--### 11/11/2021 - Modified and fine-tuned for U-M Peds AllAdm cohort only by Sarah Maidlow
--##############################################################################
--##############################################################################
/*
INTRODUCTION:
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_CONFIG_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_CONFIG_PED (
	siteid varchar(20), -- Up to 20 letters or numbers, must start with letter, no spaces or special characters.
	race_data_available number(10), -- 1 if your site collects race/ethnicity data; 0 if your site does not collect this.
	icu_data_available number(10), -- 1 if you have data on whether patients were in the ICU
	death_data_available number(10), -- 1 if you have data on whether patients have died
	...
	include_extra_cohorts_phase1 number(10), -- 0 by default, 1 to include COVID negative, U07.1, and non-admitted cohorts to Phase 1 files
	obfuscation_blur number(10), -- Add random number  || /-blur to each count (0 = no blur)
	obfuscation_small_count_mask number(10), -- Replace counts less than mask with -99 (0 = no small count masking)
	obfuscation_small_count_delete number(10), -- Delete rows where all values are small counts (0 = no, 1 = yes)
	obfuscation_agesex number(10), -- Replace combination of age-sex and total counts with -999 (0 = no, 1 = yes)
	output_phase1_as_columns number(10), -- Return the data in tables with separate columns per field
	output_phase1_as_csv number(10), -- Return the data in tables with a single column containing comma separated values
	save_phase1_as_columns number(10), -- Save the data as tables with separate columns per field
	...
	include_extra_cohorts_phase2 number(10), -- 0 by default, 1 to include COVID negative, U07.1, and non-admitted cohorts to Phase 2 files
	replace_patient_num number(10), -- Replace the patient_num with a unique random number
	output_phase2_as_columns number(10), -- Return the data in tables with separate columns per field
	output_phase2_as_csv number(10), -- Return the data in tables with a single column containing comma separated values
	save_phase2_as_columns number(10), -- Save the data as tables with separate columns per field
...
INSERT INTO I2B2REPORTING.FOURCE_CONFIG_PED
	select 'UMICH', -- siteid
		...
		-- Phase 1
--### sem - Peds does not need extra cohorts for NotAdm and COVID-Neg (commented out in code and AllAdm cohort added)
		...
		0, -- output_phase1_as_columns, default is 0
		1, -- output_phase1_as_csv
		1, -- save_phase1_as_columns, default is 0
		'I2B2DEMODATA_FOURCE_', -- save_phase1_as_prefix (don't use "4CE" since it starts with a number)
		-- Phase 2
--### sem - Peds does not need extra cohorts for NotAdm and COVID-Neg (commented out in code and AllAdm cohort added)
		1, -- include_extra_cohorts_phase2 (please set to 1 if allowed by your IRB and institution)
		0, -- replace_patient_num
		0, -- output_phase2_as_columns, default is 0
		1, -- output_phase2_as_csv
		1, -- save_phase2_as_columns, default is 0
		'I2B2DEMODATA_FOURCE_', -- save_phase2_as_prefix (don't use "4CE" since it starts with a number
        to_date('01-JAN-2019')
    from dual;
COMMIT;

--### sem - set this to the end_date/cutoff date?
-- set source_data_updated_date to whenever the DOCTR monthly ETL load finished (or end_date if there is a cutoff date???):
--UPDATE I2B2REPORTING.FOURCE_CONFIG_PED set source_data_updated_date=to_date('31-MAY-2021','dd-mon-rrrr');
UPDATE I2B2REPORTING.FOURCE_CONFIG_PED set source_data_updated_date=to_date('04-NOV-2021','dd-mon-rrrr');
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_CODE_MAP_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_CODE_MAP_PED (
	code varchar(50) not null,
	local_code varchar(50) not null
);

ALTER TABLE I2B2REPORTING.FOURCE_CODE_MAP_PED add primary key (code, local_code);
...
INSERT INTO I2B2REPORTING.FOURCE_CODE_MAP_PED
...
--    union  all select 'inpatient_inout_cd', 'EI' from dual
--    union  all select 'inpatient_inout_cd', 'E' from dual
...
INSERT INTO I2B2REPORTING.FOURCE_CODE_MAP_PED
	select '', '' from dual where 1=0
-- ### sem - U-M does NOT use location_cd
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_ICU_LOCATION CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_ICU_LOCATION as select cast(department_id as varchar2(50)) location_cd from icus;
...
INSERT INTO I2B2REPORTING.FOURCE_CODE_MAP_PED
	select distinct 'covidpos', concept_cd
		...
		where concept_path like '\ACT\UMLS_C0031437\SNOMED_3947185011\UMLS_C0022885\UMLS_C1335447\%'
			...
			and not exists (select * from I2B2REPORTING.FOURCE_CODE_MAP_PED m where m.code='covidpos' and m.local_code=c.concept_cd)
	union all
	select distinct 'covidneg', concept_cd
		...
		where concept_path like '\ACT\UMLS_C0031437\SNOMED_3947185011\UMLS_C0022885\UMLS_C1334932\%'
			and concept_cd is not null
			and not exists (select * from I2B2REPORTING.FOURCE_CODE_MAP_PED m where m.code='covidneg' and m.local_code=c.concept_cd);
COMMIT;
...
INSERT INTO I2B2REPORTING.FOURCE_CODE_MAP_PED
	select 'covidU071', code_prefix_icd10cm || 'U07.1'
		from I2B2REPORTING.FOURCE_CONFIG_PED
	union all
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LAB_MAP_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LAB_MAP_PED (
...
ALTER TABLE I2B2REPORTING.FOURCE_LAB_MAP_PED add primary key (fource_loinc, local_lab_code, local_lab_units);

INSERT INTO I2B2REPORTING.FOURCE_LAB_MAP_PED
/* original code:
...
*/
	...
	from (
		select null fource_loinc, null fource_lab_units, null fource_lab_name,
				null scale_factor, null local_lab_code, null local_lab_units, null local_lab_name from dual
			where 1=0
		union select '1742-6', 'U/L', 'alanine aminotransferase (ALT)', 1, '1742-6', 'U/L', 'alanine aminotransferase (ALT)'
from dual union select '1742-6', 'U/L', 'alanine aminotransferase (ALT)', 1, '1742-6', 'IU/L', 'alanine aminotransferase (ALT)'
--from dual union select '1742-6', 'U/L', 'alanine aminotransferase (ALT)', 1, '1742-6', 'IU/mL', 'alanine aminotransferase (ALT)'

from dual union select '1751-7', 'g/dL', 'albumin', 1, '1751-7', 'mg/dL', 'albumin'
--from dual union select '1751-7', 'g/dL', 'albumin', 1, '1751-7', 'MG/DL', 'albumin'
--from dual union select '1751-7', 'g/dL', 'albumin', 1, '2862-1', 'GM/DL', 'albumin'
from dual union select '1751-7', 'g/dL', 'albumin', 1, '2862-1', 'g/dL', 'albumin'
from dual union select '1751-7', 'g/dL', 'albumin', 1, '61151-7', 'g/dL', 'albumin'

from dual union  select '1920-8', 'U/L', 'aspartate aminotransferase (AST)', 1, '1920-8', 'U/L', 'aspartate aminotransferase (AST)'
from dual union  select '1920-8', 'U/L', 'aspartate aminotransferase (AST)', 1, '1920-8', 'IU/L', 'aspartate aminotransferase (AST)'
--from dual union  select '1920-8', 'U/L', 'aspartate aminotransferase (AST)', 1, '1920-8', 'IU/mL', 'aspartate aminotransferase (AST)'

from dual union  select '1975-2', 'mg/dL', 'total bilirubin', 1, '1975-2', 'mg/dL', 'total bilirubin'
--from dual union  select '1975-2', 'mg/dL', 'total bilirubin', 1, '1975-2', 'MG/DL', 'total bilirubin'

--			from dual union  select '1988-5', 'mg/L', 'C-reactive protein (CRP) (Normal Sensitivity)', 1, '1988-5', 'mg/L', 'C-reactive protein (CRP) (Normal Sensitivity)'
from dual union  select '1988-5', 'mg/L', 'C-reactive protein (CRP) (Normal Sensitivity)', 10, '1988-5', 'mg/dL', 'C-reactive protein (CRP) (Normal Sensitivity)'
--from dual union  select '1988-5', 'mg/L', 'C-reactive protein (CRP) (Normal Sensitivity)', 1, '1988-5', 'MG/DL', 'C-reactive protein (CRP) (Normal Sensitivity)'

from dual union  select '2019-8', 'mmHg', 'PaCO2', 1, '2019-8', 'mmHg', 'PaCO2'
from dual union  select '2019-8', 'mmHg', 'PaCO2', 1, '2019-8', 'mmhg', 'PaCO2'
from dual union  select '2019-8', 'mmHg', 'PaCO2', 1, '2020-6', 'mmHg', 'PaCO2'
from dual union  select '2019-8', 'mmHg', 'PaCO2', 1, '2020-6', 'mmhg', 'PaCO2'
--from dual union  select '2019-8', 'mmHg', 'PaCO2', 1, '2021-4', 'mmol/L', 'PaCO2'
from dual union  select '2019-8', 'mmHg', 'PaCO2', 1, '2021-4', 'mmHg', 'PaCO2'
from dual union  select '2019-8', 'mmHg', 'PaCO2', 1, '2021-4', 'mmhg', 'PaCO2'


from dual union  select '2160-0', 'mg/dL', 'creatinine', 1, '2160-0', 'mg/dL', 'creatinine'

from dual union  select '2276-4', 'ng/mL', 'Ferritin', 1, '2276-4', 'ng/mL', 'Ferritin'

--from dual union  select '2532-0', 'U/L', 'lactate dehydrogenase (LDH)', 1, '2532-0', 'U/L', 'lactate dehydrogenase (LDH)'
from dual union  select '2532-0', 'U/L', 'lactate dehydrogenase (LDH)', 1, '2532-0', 'IU/L', 'lactate dehydrogenase (LDH)'

from dual union  select '2703-7', 'mmHg', 'PaO2', 1, '2703-7', 'mmHg', 'PaO2'
from dual union  select '2703-7', 'mmHg', 'PaO2', 1, '2703-7', 'mmhg', 'PaO2'
from dual union  select '2703-7', 'mmhg', 'PaO2', 1, '2704-5', 'mmHg', 'PaO2'
from dual union  select '2703-7', 'mmhg', 'PaO2', 1, '2704-5', 'mmhg', 'PaO2'
from dual union  select '2703-7', 'mmhg', 'PaO2', 1, '2705-2', 'mmHg', 'PaO2'
from dual union  select '2703-7', 'mmhg', 'PaO2', 1, '2705-2', 'mmhg', 'PaO2'

from dual union  select '3255-7', 'mg/dL', 'Fibrinogen', 1, '3255-7', 'mg/dL', 'Fibrinogen'

from dual union  select '33959-8', 'ng/mL', 'procalcitonin', 1, '33959-8', 'ng/mL', 'procalcitonin'

from dual union  select '48065-7', 'ng/mL{FEU}', 'D-dimer (FEU)', 1, '48065-7', 'ng/mL{FEU}', 'D-dimer (FEU)'
from dual union  select '48066-5', 'ng/mL{DDU}', 'D-dimer (DDU)', 1, '48066-5', 'ng/mL{DDU}', 'D-dimer (DDU)'
from dual union  select '48065-7', 'ng/mL{FEU}', 'D-dimer (FEU)', 1000, '7799-0', 'mg/L FEU', 'D-dimer (FEU)'
--from dual union  select '48066-5', 'ng/mL{DDU}', 'D-dimer (DDU)', 1000, '7799-0', 'mg/L', 'D-dimer'
--from dual union  select '48066-5', 'ng/mL{FEU}', 'D-dimer (FEU)', 1000, '7799-0', 'mg/L FEU', 'D-dimer (FEU)'

from dual union  select '49563-0', 'ng/mL', 'cardiac troponin (High Sensitivity)', 1, '49563-0', 'ng/mL', 'cardiac troponin (High Sensitivity)'
from dual union  select '49563-0', 'ng/mL', 'cardiac troponin (High Sensitivity)', .001, '67151-1', 'pg/mL', 'cardiac troponin (High Sensitivity)'
from dual union  select '6598-7', 'ug/L', 'cardiac troponin (Normal Sensitivity)', 1, '6598-7', 'ug/L', 'cardiac troponin (Normal Sensitivity)'
from dual union  select '6598-7', 'ug/L', 'cardiac troponin (Normal Sensitivity)', 1, '10839-9', 'ug/L', 'cardiac troponin (Normal Sensitivity)'
--from dual union  select '6598-7', 'ug/L', 'cardiac troponin (Normal Sensitivity)', 1, '10839-9', 'NG/ML', 'cardiac troponin (Normal Sensitivity)'

--			from dual union  select '5902-2', 's', 'prothrombin time (PT)', 1, '5902-2', 's', 'prothrombin time (PT)'
from dual union  select '5902-2', 's', 'prothrombin time (PT)', 1, '5902-2', 'sec', 'prothrombin time (PT)'

--			from dual union  select '6690-2', '10*3/uL', 'white blood cell count (Leukocytes)', 1, '6690-2', '10*3/uL', 'white blood cell count (Leukocytes)'
from dual union  select '6690-2', '10*3/uL', 'white blood cell count (Leukocytes)', 1, '6690-2', 'K/uL', 'white blood cell count (Leukocytes)'
from dual union  select '6690-2', '10*3/uL', 'white blood cell count (Leukocytes)', 1, '6690-2', 'K/MM3', 'white blood cell count (Leukocytes)'
from dual union  select '6690-2', '10*3/uL', 'white blood cell count (Leukocytes)', 1, '12227-5', 'K/uL', 'white blood cell count (Leukocytes)'

from dual union  select '731-0', '10*3/uL', 'lymphocyte count', 1, '731-0', '10*3/uL', 'lymphocyte count'
from dual union  select '731-0', '10*3/uL', 'lymphocyte count', 1, '731-0', 'K/uL', 'lymphocyte count'
from dual union  select '731-0', '10*3/uL', 'lymphocyte count', 1, '731-0', 'K/MM3', 'lymphocyte count'

from dual union select '751-8', '10*3/uL', 'neutrophil count', 1, '751-8', '10*3/uL', 'neutrophil count'
from dual union select '751-8', '10*3/uL', 'neutrophil count', 1, '751-8', 'K/MM3', 'neutrophil count'
--from dual union select '751-8', '10*3/uL', 'neutrophil count', 1, '751-8', 'cells/mcL', 'neutrophil count'
from dual union select '751-8', '10*3/uL', 'neutrophil count', 1, '751-8', 'K/uL', 'neutrophil count'

			from dual union select '777-3', '10*3/uL', 'platelet count', 1, '777-3', '10*3/uL', 'platelet count'
from dual union select '777-3', '10*3/uL', 'platelet count', 1, '777-3', 'K/MM3', 'platelet count'
from dual union select '777-3', '10*3/uL', 'platelet count', 1, '777-3', 'K/uL', 'platelet count'

--from dual union select '34714-6', 'DEFAULT', 'INR', 1, '34714-6', 'DEFAULT', 'YourLocalLabName' from dual

from dual union select '34714-6', 'DEFAULT', 'INR', 1, '34714-6', 'DEFAULT', 'Coagulation tissue factor induced.INR:RelTime:Pt:Bld:Qn:Coag'
from dual union select '34714-6', 'DEFAULT', 'INR', 1, '46418-0', 'DEFAULT', 'Coagulation tissue factor induced.INR:RelTime:Pt:BldC:Qn:Coag' from dual
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LAB_UNITS_FACTS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LAB_UNITS_FACTS_PED (
	fact_code varchar(50) not null,
	fact_units varchar(50),
	num_facts number(10),
	mean_value number(18,5),
	stdev_value number(18,5)
);

--DROP INDEX fource_lap_map_ndx_PED;
CREATE INDEX fource_lap_map_ndx_PED on I2B2REPORTING.FOURCE_LAB_MAP_PED(local_lab_code);

INSERT INTO I2B2REPORTING.FOURCE_LAB_UNITS_FACTS_PED
select * from (
with labs_in_period as (
select concept_cd, units_cd, nval_num
	from I2B2DEMODATA.OBSERVATION_FACT f
    join I2B2REPORTING.FOURCE_LAB_MAP_PED m  on m.local_lab_code = f.concept_cd
		 and m.local_lab_units=f.units_cd
	where trunc(start_date) >= (select trunc(start_date) 
		from I2B2REPORTING.FOURCE_CONFIG_PED 
		where rownum = 1)
		    and modifier_cd = '@'
)
...
/* DCS - try to merge case-sensitive UOMs:
select * from (
with labs_in_period as (
select concept_cd, units_cd, nval_num
	from I2B2DEMODATA.OBSERVATION_FACT f
    join I2B2REPORTING.FOURCE_LAB_MAP m  on m.local_lab_code = f.concept_cd
		     and lower(m.local_lab_units)=lower(f.units_cd)
	where trunc(start_date) >= (select trunc(start_date) from I2B2REPORTING.FOURCE_CONFIG where rownum = 1)
)
select concept_cd, lower(units_cd), count(*) num_facts, avg(nval_num) avg_val, stddev(nval_num) stdev_val
from labs_in_period
group by concept_cd, lower(units_cd));
*/
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LAB_MAP_REPORT_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LAB_MAP_REPORT_PED (
	fource_loinc varchar(20) not null,
...
;
ALTER TABLE I2B2REPORTING.FOURCE_LAB_MAP_REPORT_PED add primary key (fource_loinc, local_lab_code, local_lab_units);
-- Compare the fource_lab_map table to the codes and units in the data

INSERT INTO I2B2REPORTING.FOURCE_LAB_MAP_REPORT_PED
	select
	...
	from I2B2REPORTING.FOURCE_LAB_MAP_PED m
		full outer join I2B2REPORTING.FOURCE_LAB_UNITS_FACTS_PED f
		...
		left outer join (
			...
			from I2B2REPORTING.FOURCE_LAB_MAP_PED
		) a on a.local_lab_code=f.fact_code
		left outer join (
			select distinct fact_code from I2B2REPORTING.FOURCE_LAB_UNITS_FACTS_PED
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_MED_MAP_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_MED_MAP_PED (
	med_class varchar(50) not null,
	code_type varchar(10) not null,
	local_med_code varchar(50) not null
)
;
ALTER TABLE I2B2REPORTING.FOURCE_MED_MAP_PED add primary key (med_class, code_type, local_med_code);

-- ATC codes (optional)
INSERT INTO I2B2REPORTING.FOURCE_MED_MAP_PED
...
--------------------------------------------------------------------------------
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_PROC_MAP_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_PROC_MAP_PED (
	proc_group varchar(50) not null,
	code_type varchar(10) not null,
	local_proc_code varchar(50) not null
);
ALTER TABLE I2B2REPORTING.FOURCE_PROC_MAP_PED add primary key (proc_group, code_type, local_proc_code);

-- CPT4 (United States)
INSERT INTO I2B2REPORTING.FOURCE_PROC_MAP_PED
...
--------------------------------------------------------------------------------
--drop table fource_misc_PED;
/* BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_MISC_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_MISC_PED (
	patient_num number(10) not null,
	misc_date date not null
);

ALTER TABLE I2B2REPORTING.FOURCE_MISC_PED add primary key (patient_num);

INSERT INTO I2B2REPORTING.FOURCE_MISC_PED
...
--------------------------------------------------------------------------------
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_COHORT_CONFIG_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_COHORT_CONFIG_PED (
	cohort varchar(50) not null,
	include_in_phase1 number(10), -- 1 = include the cohort in the phase 1 output, otherwise 0
	include_in_phase2 number(10), -- 1 = include the cohort in the phase 2 output and saved files, otherwise 0
...
ALTER TABLE I2B2REPORTING.FOURCE_COHORT_CONFIG_PED add primary key (cohort);
-- ### sem - We only need this cohort for the Peds AllAdm project
insert into I2B2REPORTING.FOURCE_COHORT_CONFIG_PED
	select 'AllAdm', 1, 1, NULL, NULL, NULL from dual;
commit;

-- Assume the data were updated on the date this script is run if source_data_updated_date is null
UPDATE I2B2REPORTING.FOURCE_COHORT_CONFIG_PED
    set source_data_updated_date = to_date('04-NOV-2021','dd-mon-rrrr')
--	set source_data_updated_date = nvl((select source_data_updated_date from I2B2REPORTING.FOURCE_CONFIG_PED),sysdate)
--	where source_data_updated_date is null;
;
COMMIT;

--### sem - used for the eval_start_date in the fource_ped_config to update earliest_adm_date permitted
update I2B2REPORTING.FOURCE_COHORT_CONFIG_PED
    set earliest_adm_date = to_date('01-JAN-2019','dd-mon-rrrr')
--  set earliest_adm_date = nvl((select eval_start_date from fource_ped_config),sysdate)
--	where earliest_adm_date is null;
 ;   
commit;

--### sem - used for the eval_end_date in the fource_ped_config to update latest_adm_date permitted
update I2B2REPORTING.FOURCE_COHORT_CONFIG_PED
    set latest_adm_date = to_date('31-MAY-2021','dd-mon-rrrr')
--  set latest_adm_date = nvl((select source_data_updated_date from fource_ped_config),sysdate)
--	where latest_adm_date is null;
 ;   
commit;


--########################################################################################	
-- sem - commented out for Peds AllAdm above 
/*
INSERT INTO I2B2REPORTING.FOURCE_COHORT_CONFIG_PED
...
*/
--########################################################################################	
-- these cohorts are added in the SQL in the sections of code body for %NotAdm%	
--	union all select 'PosNotAdm2020Q1', 0, 0, NULL, '01-JAN-2020', '31-MAR-2020' from dual
--	union all select 'PosNotAdm2020Q2', 0, 0, NULL, '01-APR-2020', '30-JUN-2020' from dual
--	union all select 'PosNotAdm2020Q3', 0, 0, NULL, '01-JUL-2020', '30-SEP-2020' from dual
--	union all select 'PosNotAdm2020Q4', 0, 0, NULL, '01-OCT-2020', '31-DEC-2020' from dual
--	union all select 'PosNotAdm2021Q1', 0, 0, NULL, '01-JAN-2021', '31-MAR2021' from dual
--	union all select 'PosNotAdm2021Q2', 0, 0, NULL, '01-APR-2021', '30-JUN-2021' from dual
--	union all select 'PosNotAdm2021Q3', 0, 0, NULL, '01-JUL-2021', '30-SEP-2021' from dual
--	union all select 'PosNotAdm2021Q4', 0, 0, NULL, '01-OCT-2021', '31-DEC-2021' from dual
--	union all select 'OtherAdm', 0, 0, NULL, '01-JAN-2020', '31-DEC-2021' from dual
--	union all select 'OtherNotAdm', 0, 0, NULL, '01-JAN-2020', '31-DEC-2021' from dual
--	;
--COMMIT;
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_COVID_TESTS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_COVID_TESTS_PED (
	patient_num number(10) not null,
	test_result varchar(10) not null,
	test_date date not null
);

ALTER TABLE I2B2REPORTING.FOURCE_COVID_TESTS_PED add primary key (patient_num, test_result, test_date);

INSERT INTO I2B2REPORTING.FOURCE_COVID_TESTS_PED
	select distinct f.patient_num, m.code, trunc(start_date)
		from I2B2DEMODATA.OBSERVATION_FACT f --with (nolock)
			inner join I2B2REPORTING.FOURCE_CODE_MAP_PED m
...
--### sem - updates to include patient_dim and use age filter
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_ADMISSIONS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_ADMISSIONS_PED (
	patient_num number(10) not null,
	admission_date date not null,
	discharge_date date not null
);
 
ALTER TABLE I2B2REPORTING.FOURCE_ADMISSIONS_PED add primary key (patient_num, admission_date, discharge_date);

INSERT INTO I2B2REPORTING.FOURCE_ADMISSIONS_PED
  	select distinct patient_num, cast(start_date as date), nvl(cast(end_date as date),'01-JAN-2199') -- a very future date for missing discharge dates
		from (
		select vd.patient_num, trunc(iv.hosp_admsn_time) start_date, trunc(iv.hosp_disch_time) end_date
			from I2B2DEMODATA.VISIT_DIMENSION vd, --join with the inpatient_visits to get the admission/discharge date like how currently works
			I2B2DEMODATA.INPATIENT_VISITS iv,
			i2b2demodata.patient_dimension p -- ### sem
			where vd.patient_num=iv.patient_num
			and vd.encounter_num=iv.encounter_num
			and iv.admitted='Y'
			and iv.patient_num = p.patient_num -- ### sem
			and p.age_in_years_num <= '25' -- ### sem
--			and trunc(iv.start_date) >= (select trunc(eval_start_date) from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1)
			and trunc(iv.hosp_admsn_time) >= (select trunc(eval_start_date) from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1)
--### sem - added comparison dates for Peds AllAdm
        and (iv.hosp_admsn_time <=  (select trunc(SOURCE_DATA_UPDATED_DATE) from fource_ped_config where rownum = 1))
        and (iv.hosp_disch_time >= iv.hosp_admsn_time)
--			and vd.patient_num in 
--				(select patient_num from I2B2REPORTING.FOURCE_COVID_TESTS_PED)
			and vd.inout_cd in 
				(select local_code from I2B2REPORTING.FOURCE_CODE_MAP_PED where code = 'inpatient_inout_cd')
/*		union all
			select iv.patient_num, trunc(iv.start_date), nvl(trunc(iv.end_date),trunc(v.end_date))
			from I2B2DEMODATA.INPATIENT_VISITS iv,
			I2B2DEMODATA.VISIT_DIMENSION v,
			i2b2demodata.patient_dimension p, --### sem
			I2B2DEMODATA.OBSERVATION_FACT obf
				where v.encounter_num=iv.encounter_num and v.patient_num=iv.patient_num
				and v.patient_num = p.patient_num --### sem
				and iv.admitted='Y'
				and p.age_in_years_num <= '25' --### sem 
				and iv.patient_num=obf.patient_num and iv.encounter_num=obf.encounter_num
				and trunc(iv.start_date) >= 
					(select trunc(eval_start_date) from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1)
				and iv.patient_num in 
					(select distinct(patient_num) from I2B2REPORTING.FOURCE_COVID_TESTS_PED)
				-- this part still needs work as the LOCAL_CODE is a custom CONCEPT_CD:
				and substr(obf.concept_cd,6) in 
					(select local_code from I2B2REPORTING.FOURCE_CODE_MAP_PED where code = 'inpatient_concept_cd')
*/
			) t;
COMMIT;
...
--------------------------------------------------------------------------------
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_ICU_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_ICU_PED (
	patient_num number(10) not null,
	start_date date not null,
	end_date date not null
);

ALTER TABLE I2B2REPORTING.FOURCE_ICU_PED add primary key (patient_num, start_date, end_date);
--truncate table fource_icu;
INSERT INTO I2B2REPORTING.FOURCE_ICU_PED
...
-- Not reliable at U-M -- use the 4099_ICU_FINAL table below
/* 			select patient_num, trunc(start_date) start_date, trunc(end_date) end_date
				from I2B2DEMODATA.VISIT_DIMENSION
				where trunc(start_date) >= (select trunc(eval_start_date) from I2B2REPORTING.FOURCE_CONFIG where rownum = 1)
					and patient_num in (select patient_num from I2B2REPORTING.FOURCE_COVID_TESTS)
					and inout_cd in (select local_code from I2B2REPORTING.FOURCE_CODE_MAP where code = 'icu_inout_cd')
			-- Select by location_cd
-- We do not use location_cd in VISIT_DIMENSION and instead use the 4099_ICU_FINAL table below
			select patient_num, trunc(start_date) start_date, trunc(end_date) end_date --***** SOMEONE PLEASE AUDIT THIS SECTION******
				from I2B2DEMODATA.VISIT_DIMENSION v
				where trunc(start_date) >= (select trunc(eval_start_date) from I2B2REPORTING.FOURCE_CONFIG where rownum = 1)
					and patient_num in (select patient_num from I2B2REPORTING.FOURCE_COVID_TESTS)
					and location_cd in (select local_code from I2B2REPORTING.FOURCE_CODE_MAP where code = 'icu_location_cd')
			union all  */		
		-- Select by concept_cd 
-- ### sem - not recommend to use. The dates are not accurate for inpatient admissions in the OBSERVATION_FACT table
/*				select f.patient_num, trunc(f.start_date) start_date, nvl(trunc(f.end_date),trunc(v.end_date)) end_date
				from I2B2DEMODATA.OBSERVATION_FACT f
					inner join I2B2DEMODATA.VISIT_DIMENSION v
						on v.encounter_num=f.encounter_num and v.patient_num=f.patient_num 
				where trunc(f.start_date) >= (select trunc(eval_start_date) from fource_ped_config where rownum = 1)
					and f.concept_cd in (select local_code from fource_ped_code_map where code = 'icu_concept_cd')
                    and f.patient_num in (select patient_num from fource_admissions_PED) --### must use the admsissions table here
 				where trunc(f.start_date) >= (select trunc(eval_start_date) from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1)
					and f.patient_num in (select patient_num from I2B2REPORTING.FOURCE_COVID_TESTS_PED)
					and f.patient_num in
						(select patient_num from i2b2demodata.patient_dimension p
						 where p.age_in_years_num <= '25')
					and f.concept_cd in (select local_code from I2B2REPORTING.FOURCE_CODE_MAP_PED where code = 'icu_concept_cd')
				union all
            -- Select by location_cd in observation_fact table
-- ### sem - U-M does not use location_cd in Observation_Fact see HPI_4099_ICU_FINAL table used below for ICU patients
			...
			--		inner join I2B2DEMODATA.VISIT_DIMENSION v
			--		on v.encounter_num=f.encounter_num and v.patient_num=f.patient_num
                --  inner join I2B2REPORTING.FOURCE_ICU_LOCATION l on l.location_cd = f.location_cd
				-- 	where trunc(f.start_date) >= (select trunc(eval_start_date) from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1)
				--	and f.patient_num in (select patient_num from I2B2REPORTING.FOURCE_COVID_TESTS_PED)
				--	and f.patient_num in(select patient_num from i2b2demodata.patient_dimension p
				--			where p.age_in_years_num <= '25')
...
*/
				select vd.patient_num, trunc(hpi.icu_start_date) start_date, trunc(hpi.icu_end_date) end_date
				from I2B2DEMODATA.HPI_4099_ICU_FINAL hpi,
							I2B2DEMODATA.VISIT_DIMENSION vd
				where hpi.patient_num=vd.patient_num
				and hpi.encounter_num=vd.encounter_num
				and trunc(hpi.icu_start_date) >= (select trunc(eval_start_date) from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1)
				and vd.patient_num in (select patient_num from fource_admissions_PED) --### must use the 4CE admsissions table here
--					and vd.inout_cd in (select local_code from I2B2REPORTING.FOURCE_CODE_MAP where code = 'icu_inout_cd')
       ) t
        where (select icu_data_available from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) = 1;
COMMIT;

DELETE FROM I2B2REPORTING.FOURCE_ICU_PED where trunc(end_date) < trunc(start_date);
COMMIT;
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_DEATH_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_DEATH_PED (
	patient_num number(10) not null,
	death_date date not null
);

ALTER TABLE I2B2REPORTING.FOURCE_DEATH_PED add primary key (patient_num);

-- The death_date is estimated later in the SQL if it is null here.
INSERT INTO I2B2REPORTING.FOURCE_DEATH_PED
		...
		where (death_date is not null or vital_status_cd in ('Y', 'DEM|VITAL STATUS:D'))
			and patient_num in (select patient_num from fource_admissions_PED) --### must use the 4CE admsissions table here
            )t
    where (select death_data_available from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) = 1
;
COMMIT;
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_FIRST_COVID_TESTS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_FIRST_COVID_TESTS_PED (
	patient_num number(10) not null,
	first_pos_date date,
	first_neg_date date,
...
ALTER TABLE I2B2REPORTING.FOURCE_FIRST_COVID_TESTS_PED add primary key (patient_num);

INSERT INTO I2B2REPORTING.FOURCE_FIRST_COVID_TESTS_PED
	select patient_num,
		...
		from I2B2REPORTING.FOURCE_COVID_TESTS_PED
		group by patient_num;
COMMIT;
--*/

--  LOOK AT OTHER OPTIONAL COHORTS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
-- ############################################################################################################
--------------------------------------------------------------------------------
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED (
	cohort varchar(50) not null,
	patient_num number(10) not null,
	admission_date date not null,
	source_data_updated_date date not null,
	severe number(10) not null,
	severe_date date,
	death_date date
);

ALTER TABLE I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED add primary key (patient_num, cohort);

--select * from fource_cohort_config_PED;

-- ############ sem - changed this for Peds AllAdm  ############################################################################################
insert into I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED 
	select c.cohort, a.patient_num, min(a.admission_date), c.source_data_updated_date, 0, null, null
	from I2B2REPORTING.FOURCE_ADMISSIONS_PED a
		inner join I2B2REPORTING.FOURCE_COHORT_CONFIG_PED c
...
COMMIT;
 ...
 /*  original: commented out for Peds AllAdm
 INSERT INTO I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED (cohort, patient_num, admission_date, source_data_updated_date, severe)
	select c.cohort, t.patient_num, t.admission_date, c.source_data_updated_date, 0
	from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED c,		(
			select t.patient_num, min(a.admission_date) admission_date
--			from I2B2REPORTING.FOURCE_FIRST_COVID_TESTS_PED t
				inner join I2B2REPORTING.FOURCE_ADMISSIONS_PED a
...
--select * from fource_cohort_patients_PED;
*/

/*
-- ### sem - The Additional Cohorts below are not needed for Peds AllAdm ##########################################
--------------------------------------------------------------------------------
...
INSERT INTO I2B2REPORTING.FOURCE_COHORT_CONFIG_PED
		...
		select replace(c.cohort,'PosAdm','U071Adm'),
			...
			from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED c cross apply I2B2REPORTING.FOURCE_CONFIG_PED g
			where c.cohort like 'PosAdm%'
		...
		select replace(c.cohort,'PosAdm','NegAdm'),
			...
			from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED c cross apply I2B2REPORTING.FOURCE_CONFIG_PED g
			where c.cohort like 'PosAdm%')
--        where (select include_extra_cohorts_phase1 from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) = 1
--        or (select include_extra_cohorts_phase2 from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) = 1
		;
COMMIT;
...
--#### sem - Peds does NOT need patients NotAdm
...
INSERT INTO I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED (cohort, patient_num, admission_date, source_data_updated_date, severe)
		select c.cohort, t.patient_num, t.first_pos_date, c.source_data_updated_date, 0
			from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED c
				cross join I2B2REPORTING.FOURCE_FIRST_COVID_TESTS_PED t
			where c.cohort like 'PosNotAdm%'
				...
				and t.patient_num not in (select patient_num from I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED)
	                and t.patient_num in (select p.patient_num from I2B2demodata.patient_dimension p
                        where p.age_in_years_num <='25')			
		...
		select c.cohort, t.patient_num, t.first_neg_date, c.source_data_updated_date, 0
			from I2B2REPORTING.FOURCE_COHORT_CONFIG c
				cross join I2B2REPORTING.FOURCE_FIRST_COVID_TESTS t
			where c.cohort like 'NegNotAdm%'
				...
				and t.patient_num not in (select patient_num from I2B2REPORTING.FOURCE_COHORT_PATIENTS);
...
-- My custom cohorts
-- taken from 4CE_PhaseX.2_CustomCohorts_AllPatients_oracle.sql
...
INSERT INTO I2B2REPORTING.FOURCE_ADMISSIONS_PED
    select *
        from (
        select distinct patient_num, cast(start_date as date) admission_date, nvl(cast(end_date as date),'01-JAN-2199') discharge_date
            from (
                select patient_num, start_date, end_date
                    from I2B2DEMODATA.VISIT_DIMENSION
                    where inout_cd in (select local_code from I2B2REPORTING.FOURCE_CODE_MAP_PED where code = 'inpatient_inout_cd')
                union all
                select patient_num, start_date, end_date
                    from I2B2DEMODATA.VISIT_DIMENSION v
                    where location_cd in (select local_code from I2B2REPORTING.FOURCE_CODE_MAP_PED where code = 'inpatient_location_cd')
                union all
                select f.patient_num, f.start_date, nvl(f.end_date,v.end_date)
                    from I2B2DEMODATA.OBSERVATION_FACT f
                        inner join I2B2DEMODATA.VISIT_DIMENSION v
                            on v.encounter_num=f.encounter_num and v.patient_num=f.patient_num
                    where f.concept_cd in (select local_code from I2B2REPORTING.FOURCE_CODE_MAP_PED where code = 'inpatient_concept_cd')
            ) t
        ) t
        where (admission_date >= '01-JAN-2019') and (discharge_date >= admission_date)
            and patient_num not in (select patient_num from I2B2REPORTING.FOURCE_COVID_TESTS);
--#############################################################################################################################
...
-- Add ICU dates for other patients
  	INSERT INTO I2B2REPORTING.FOURCE_ICU_PED
							select *
							from (
								select distinct patient_num, cast(start_date as date) start_date, nvl(cast(end_date as date),'01-JAN-2199') end_date
								from (
									select patient_num, start_date, end_date
										from I2B2DEMODATA.VISIT_DIMENSION
										where inout_cd in (select local_code from I2B2REPORTING.FOURCE_CODE_MAP_PED where code = 'icu_inout_cd')
									union all
									select patient_num, start_date, end_date
										from I2B2DEMODATA.VISIT_DIMENSION v
										where location_cd in (select local_code from I2B2REPORTING.FOURCE_CODE_MAP_PED where code = 'icu_location_cd')
									union all
									select f.patient_num, f.start_date, nvl(f.end_date,v.end_date)
										from I2B2DEMODATA.OBSERVATION_FACT f
											inner join I2B2DEMODATA.VISIT_DIMENSION v
												on v.encounter_num=f.encounter_num and v.patient_num=f.patient_num
										where f.concept_cd in (select local_code from I2B2REPORTING.FOURCE_CODE_MAP_PED where code = 'icu_concept_cd')
								) t
							) t
							where (start_date >= '01-JAN-2019') and (end_date >= start_date)
--								and patient_num not in (select patient_num from I2B2REPORTING.FOURCE_COVID_TESTS_PED)
...
--										and patient_num not in (select patient_num from I2B2REPORTING.FOURCE_COVID_TESTS_PED)
										and death_date >= '01-JAN-2019'
										
--################################################################################################################################################################################			
										and age_in_years_num <= '25';
...
INSERT INTO I2B2REPORTING.FOURCE_COHORT_CONFIG_PED
								with t as (
									select '01-JAN-2019' start_date, nvl(source_data_updated_date,sysdate) end_date
									from I2B2REPORTING.FOURCE_CONFIG_PED
								    )
							-- Peds only needs admitted patients
							--### sem - but do we need all of the non-COVID admissions? ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
									select 'OtherAdm', 1, 1, end_date, start_date, end_date from t
									;
--									union all
--									select 'OtherNotAdm', 0, 0, end_date, start_date, end_date from t;
...
--### sem - Do we need all of the non-COVID admissions? ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
									from I2B2REPORTING.FOURCE_ADMISSIONS_PED a
										inner join I2B2REPORTING.FOURCE_COHORT_CONFIG_PED c
											on a.admission_date >= c.earliest_adm_date and a.admission_date <= c.latest_adm_date
									where c.cohort='OtherAdm'
										and a.patient_num not in (select patient_num from I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED)
									group by c.cohort, a.patient_num, c.source_data_updated_date;

COMMIT;

--### PEDS does NOT need OUTPTS or NotAdm
								-- Get all other patients who have an encounter
 INSERT INTO I2B2REPORTING.FOURCE_COHORT_PATIENTS
									select c.cohort, v.patient_num, min(v.start_date), c.source_data_updated_date, 0, null, null
									from I2B2DEMODATA.VISIT_DIMENSION v
										inner join I2B2REPORTING.FOURCE_COHORT_CONFIG c
											on v.start_date >= c.earliest_adm_date and v.start_date <= c.latest_adm_date
									where c.cohort='OtherNotAdm'
										and v.patient_num not in (select patient_num from I2B2REPORTING.FOURCE_COHORT_PATIENTS)
									group by c.cohort, v.patient_num, c.source_data_updated_date;

COMMIT;

--commit;
-- ### sem - end comment of extra cohorts NOT needed for Peds AllAdm project ###########################################################################
*/
--******************************************************************************
--******************************************************************************
...
-- Get a distinct list of patients -- ###sem - with age and sex ####################################
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_PATIENTS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_PATIENTS_PED (
	patient_num number(10) not null,
	first_admission_date date not null,
    age_in_years_num int not null,
    sex_cd varchar(50) not null
);
ALTER TABLE I2B2REPORTING.FOURCE_PATIENTS_PED add primary key (patient_num);

INSERT INTO I2B2REPORTING.FOURCE_PATIENTS_PED
	select p.patient_num, min(p.admission_date), v.age_in_years_num, v.sex_cd
		from fource_cohort_patients_PED p
            inner join i2b2demodata.patient_dimension v
        		on p.patient_num = v.patient_num  
				where v.age_in_years_num <= 25
    group by p.patient_num, v.age_in_years_num, v.sex_cd;
--	select patient_num, min(admission_date)
--		from I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED
--		group by patient_num;
COMMIT;
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_OBSERVATIONS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_OBSERVATIONS_PED (
	cohort varchar(50) not null,
	patient_num number(10) not null,
	severe number(10) not null,
	...
	days_since_admission number(10) not null,
	value number(18,5) not null,
	logvalue number(18,10) not null
);

ALTER TABLE I2B2REPORTING.FOURCE_OBSERVATIONS_PED add primary key (cohort, patient_num, concept_type, concept_code, days_since_admission);
--------------------------------------------------------------------------------
-- Add covid tests
--------------------------------------------------------------------------------
INSERT INTO I2B2REPORTING.FOURCE_OBSERVATIONS_PED (cohort, patient_num, severe, concept_type, concept_code, calendar_date, days_since_admission, value, logvalue)
	select distinct
		p.cohort,
		p.patient_num,
		p.severe,
		...
		-999,
		-999
 	from I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED p
		inner join I2B2REPORTING.FOURCE_COVID_TESTS_PED t
...
INSERT INTO I2B2REPORTING.FOURCE_OBSERVATIONS_PED (cohort, patient_num, severe, concept_type, concept_code, calendar_date, days_since_admission, value, logvalue)
	select distinct
		...
		-999
 	from I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED p
		inner join I2B2REPORTING.FOURCE_MISC f --with (nolock)
			on p.patient_num=f.patient_num; 
*/

-- DROP index fource_cohort_patients_ndx_PED;
CREATE INDEX fource_cohort_patients_ndx_PED on I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED(patient_num);
COMMIT;
...
INSERT INTO I2B2REPORTING.FOURCE_OBSERVATIONS_PED (cohort, patient_num, severe, concept_type, concept_code, calendar_date, days_since_admission, value, logvalue)
	select distinct
		...
		-999
 	from I2B2DEMODATA.OBSERVATION_FACT f --with (nolock)
		inner join I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED p
	...
	where f.concept_cd like (select code_prefix_icd9cm || '%' from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1);-- and code_prefix_icd9cm <>'';
COMMIT;
 	...
 	from I2B2DEMODATA.OBSERVATION_FACT f --with (nolock)
		inner join I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED p
			on f.patient_num=p.patient_num
				--and cast(trunc(f.start_date) as date) between dateadd(dd,@lookback_days,p.admission_date) and p.source_data_updated_date
                and trunc(f.start_date) between trunc(p.admission_date)-365 and trunc(p.source_data_updated_date)
	where f.concept_cd like (select code_prefix_icd10cm || '%' from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1);-- and code_prefix_icd10cm <>'';
COMMIT;
...
INSERT INTO I2B2REPORTING.FOURCE_OBSERVATIONS_PED (cohort, patient_num, severe, concept_type, concept_code, calendar_date, days_since_admission, value, logvalue)
	select distinct
		...
		-999,
		-999
	from I2B2REPORTING.FOURCE_MED_MAP_PED m
		inner join I2B2DEMODATA.OBSERVATION_FACT f --with (nolock)
...
INSERT INTO I2B2REPORTING.FOURCE_OBSERVATIONS_PED (cohort, patient_num, severe, concept_type, concept_code, calendar_date, days_since_admission, value, logvalue)
	...
	from I2B2REPORTING.FOURCE_LAB_MAP_PED l
		inner join I2B2DEMODATA.OBSERVATION_FACT f --with (nolock)
...
INSERT INTO I2B2REPORTING.FOURCE_OBSERVATIONS_PED (cohort, patient_num, severe, concept_type, concept_code, calendar_date, days_since_admission, value, logvalue)
	select distinct
		...
		-999,
		-999
 	from I2B2REPORTING.FOURCE_PROC_MAP_PED x
		inner join I2B2DEMODATA.OBSERVATION_FACT f --with (nolock)
...
INSERT INTO I2B2REPORTING.FOURCE_OBSERVATIONS_PED (cohort, patient_num, severe, concept_type, concept_code, calendar_date, days_since_admission, value, logvalue)
	...
	select cohort, patient_num, severe, 'SEVERE-LAB' concept_type, 'BloodGas' concept_code, calendar_date, days_since_admission, avg(value), avg(logvalue)
		from I2B2REPORTING.FOURCE_OBSERVATIONS_PED
...
UPDATE I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED p set severe = 1,
    severe_date=(select min(f.calendar_date)
                    from I2B2REPORTING.FOURCE_OBSERVATIONS_PED f
    ...
    where exists (
    select min(f.calendar_date) from I2B2REPORTING.FOURCE_OBSERVATIONS_PED f where f.cohort=p.cohort and f.patient_num=p.patient_num and
...
-- ### sem - update the severity flag in the fource_observations table
update fource_observations_PED f
set f.severe=1
where 
    exists(
        select patient_num, cohort
        from fource_cohort_patients_PED c
        where 
            c.severe=0 and 
            f.patient_num = c.patient_num and 
            f.cohort = c.cohort
    );
...
--begin;
	...
	MERGE INTO I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED c
    using (
		...
		from I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED c
			inner join I2B2REPORTING.FOURCE_DEATH_PED p
				on p.patient_num = c.patient_num
        group by p.patient_num) d
        on (c.patient_num = d.patient_num and
        (select death_data_available from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1)= 1)
        WHEN MATCHED THEN
...
COMMIT;
 ...
 -- ****Be sure that the future facts aren't orders, future appointmnents or late arriving lab results
	...
	using (	select p.patient_num, cast(max(f.calendar_date) as date) death_date
				from I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED p
					inner join I2B2REPORTING.FOURCE_OBSERVATIONS_PED f
	...
	UPDATE I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED
		set death_date = null
		where death_date > source_data_updated_date
        and (select death_data_available from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1)= 1;
COMMIT;
...
--******************************************************************************
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_DATE_LIST_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_DATE_LIST_PED (
	cohort varchar(50) not null,
	d date not null
);

ALTER TABLE I2B2REPORTING.FOURCE_DATE_LIST_PED add primary key (cohort, d);

INSERT INTO I2B2REPORTING.FOURCE_DATE_LIST_PED select * from (
    with n as (
	...
	) l inner join I2B2REPORTING.FOURCE_COHORT_CONFIG_PED f on l.cohort=f.cohort
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALPATIENTCLINICALCOURSE_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LOCALPATIENTCLINICALCOURSE_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	patient_num number(10) not null,
	days_since_admission number(10) not null,
	calendar_date date not null,
	in_hospital number(10) not null,
	severe number(10) not null,
	in_icu number(10) not null,
	dead number(10) not null
);

ALTER TABLE I2B2REPORTING.FOURCE_LOCALPATIENTCLINICALCOURSE_PED add primary key (cohort, patient_num, days_since_admission, siteid);
-- Get the list of dates and flag the ones where the patients were severe or deceased
INSERT INTO I2B2REPORTING.FOURCE_LOCALPATIENTCLINICALCOURSE_PED
    (siteid, cohort, patient_num, days_since_admission, calendar_date, in_hospital, severe, in_icu, dead)
	select (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) siteid,
		...
		max(case when (select icu_data_available from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1)=0 then -999 else 0 end) in_icu,
		max(case when (select death_data_available from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1)=0 then -999
			when p.death_date is not null and trunc(d.d) >= trunc(p.death_date) then 1
			else 0 end) dead
--	from fource_config x
		from I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED p
		inner join I2B2REPORTING.FOURCE_DATE_LIST_PED d
...
MERGE INTO I2B2REPORTING.FOURCE_LOCALPATIENTCLINICALCOURSE_PED p
	using (
    select distinct p.patient_num,  p.calendar_date
    from I2B2REPORTING.FOURCE_LOCALPATIENTCLINICALCOURSE_PED p
    inner join I2B2REPORTING.FOURCE_ADMISSIONS_PED a on a.patient_num = p.patient_num
...
MERGE INTO I2B2REPORTING.FOURCE_LOCALPATIENTCLINICALCOURSE_PED p
    ...
    select distinct p.patient_num,  p.calendar_date, p.in_hospital, p.in_icu
            from I2B2REPORTING.FOURCE_LOCALPATIENTCLINICALCOURSE_PED p
			inner join I2B2REPORTING.FOURCE_ICU_PED i
				on i.patient_num=p.patient_num
                    ...
                    and (select icu_data_available from I2B2REPORTING.FOURCE_CONFIG_PED where rownum=1)=1 ))d
...
--select count(distinct patient_num) from fource_LocalPatientClinicalCourse where in_icu = 1;
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	patient_num number(10) not null,
	admission_date date not null,
	...
	still_in_hospital number(10) not null,
	severe_date date not null,
	severe number(10) not null,
	icu_date date not null,
	icu number(10) not null,
	death_date date not null,
	dead number(10) not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY_PED add primary key (cohort, patient_num, siteid);
-- Get the admission, severe, and death dates; and age and sex.
INSERT INTO I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY_PED
	select (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum=1), c.cohort, c.patient_num,
        c.admission_date,
		...
		(case when (select icu_data_available from I2B2REPORTING.FOURCE_CONFIG_PED where rownum=1)=0 then -999 else 0 end) in_icu,
		nvl(c.death_date,'01-JAN-1900') death_date,
		(case when (select death_data_available from I2B2REPORTING.FOURCE_CONFIG_PED where rownum=1)=0 then -999 when c.death_date is not null then 1 else 0 end) dead,
		(case
	...
	from I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED c
		left outer join I2B2DEMODATA.PATIENT_DIMENSION p
			on p.patient_num=c.patient_num
		left outer join I2B2REPORTING.FOURCE_CODE_MAP_PED m
			on p.sex_cd = m.local_code
				and m.code in ('sex_patient:male','sex_patient:female');
COMMIT;

-- select * from fource_LocalPatientSummary_PED;

--################################################################################################################################################################################

-- Update sex if sex stored in observation_fact table
MERGE INTO I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY s
using (select p.sex, p.patient_num
	   from I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY s
		inner join (
			...
			from (
				...
				from I2B2DEMODATA.OBSERVATION_FACT f --with (nolock)
					inner join I2B2REPORTING.FOURCE_CODE_MAP m
...
-- Get the last discharge date and whether the patient is still in the hospital as of the source_data_updated_date.
	MERGE INTO I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY_PED s
	using ( select p.cohort, p.patient_num, max(a.discharge_date) last_discharge_date
			from I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY_PED p
				inner join I2B2REPORTING.FOURCE_ADMISSIONS_PED a
...
MERGE INTO I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY_PED s
      using (
			select cohort, patient_num, min(calendar_date) icu_date
					from I2B2REPORTING.FOURCE_LOCALPATIENTCLINICALCOURSE_PED
					where in_icu=1
					group by cohort, patient_num
			) x
        on (s.cohort=x.cohort and s.patient_num=x.patient_num and (select icu_data_available from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1)=1)
        when matched then
...
--------------------------------------------------------------------------------
--drop table fource_LocalPatientObservations_PED;
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALPATIENTOBSERVATIONS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LOCALPATIENTOBSERVATIONS_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	patient_num number(10) not null,
	days_since_admission number(10) not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_LOCALPATIENTOBSERVATIONS_PED add primary key (cohort, patient_num, days_since_admission, concept_type, concept_code, siteid);
INSERT INTO I2B2REPORTING.FOURCE_LOCALPATIENTOBSERVATIONS_PED
	select (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1),
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALPATIENTRACE_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LOCALPATIENTRACE_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	patient_num number(10) not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_LOCALPATIENTRACE_PED add primary key (cohort, patient_num, race_local_code, siteid);
INSERT INTO I2B2REPORTING.FOURCE_LOCALPATIENTRACE_PED
		select distinct (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) siteid, cohort, patient_num, race_local_code, race_4ce
		from (
			...
			select c.cohort, c.patient_num, m.local_code race_local_code, substr(m.code,14,999) race_4ce
				from I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED c
					inner join I2B2DEMODATA.PATIENT_DIMENSION p
						on p.patient_num=c.patient_num
					inner join I2B2REPORTING.FOURCE_CODE_MAP_PED m
			...
			select c.cohort, c.patient_num, m.local_code race_local_code, substr(m.code,11,999) race_4ce
				from I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED c
					inner join I2B2DEMODATA.OBSERVATION_FACT p --with (nolock)
						on p.patient_num=c.patient_num
					inner join I2B2REPORTING.FOURCE_CODE_MAP_PED m
						on p.concept_cd = m.local_code
							and m.code like 'race_fact:%'
		) t
        where (select race_data_available from I2B2REPORTING.FOURCE_CONFIG_PED where rownum =1)=1;
COMMIT;
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALCOHORTS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LOCALCOHORTS_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	include_in_phase1 number(10) not null,
	include_in_phase2 number(10) not null,
	source_data_updated_date date not null,
	earliest_adm_date date not null,
	latest_adm_date date not null
);
ALTER TABLE I2B2REPORTING.FOURCE_LOCALCOHORTS_PED add primary key (cohort, siteid);
INSERT INTO I2B2REPORTING.FOURCE_LOCALCOHORTS_PED
	select (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) siteid, cohort, include_in_phase1, include_in_phase2, source_data_updated_date, earliest_adm_date, latest_adm_date
	from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED;
COMMIT;
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALDAILYCOUNTS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LOCALDAILYCOUNTS_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	calendar_date date not null,
	cumulative_pts_all number(10) not null,
	cumulative_pts_icu number(10) not null,
	cumulative_pts_dead number(10) not null,
	cumulative_pts_severe number(10) not null,
	cumulative_pts_severe_icu number(10) not null,
	cumulative_pts_severe_dead number(10) not null,
	pts_in_hosp_on_this_date number(10) not null,
	pts_in_icu_on_this_date number(10) not null,
	pts_severe_in_hosp_on_date number(10) not null,
	pts_severe_in_icu_on_date number(10) not null
);

ALTER TABLE I2B2REPORTING.FOURCE_LOCALDAILYCOUNTS_PED add primary key (cohort, calendar_date, siteid);
-- Get daily counts, except for ICU

INSERT INTO I2B2REPORTING.FOURCE_LOCALDAILYCOUNTS_PED
	select (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) siteid, cohort, calendar_date,
		-- Cumulative counts
		count(*),
	...
	from I2B2REPORTING.FOURCE_CONFIG_PED x
		cross join I2B2REPORTING.FOURCE_LOCALPATIENTCLINICALCOURSE_PED c
	group by cohort, calendar_date, icu_data_available, death_data_available;
COMMIT;

-- ### sem - Adding cummulative_pts_icu in FOURCE_LOCALDAILYCOUNTS table

MERGE INTO FOURCE_LOCALDAILYCOUNTS_PED c
USING (SELECT 
    calendar_date, cohort, SUM(cumm_daily_count) cumm_daily_count 
FROM (
        SELECT DISTINCT 
            d.calendar_date, d.cohort, a.icu_date, COUNT(*) cumm_daily_count
        FROM 
            FOURCE_LOCALDAILYCOUNTS_PED d
        JOIN fource_LocalPatientSummary_PED a
...
GROUP BY calendar_date, cohort)x ON (x.cohort = c.cohort and x.calendar_date=c.calendar_date)
when matched then
update set c.cumulative_pts_icu = x.cumm_daily_count;

-- ### sem - Adding cumulative_pts_severe_icu in FOURCE_LOCALDAILYCOUNTS table

MERGE INTO FOURCE_LOCALDAILYCOUNTS_PED c
USING (SELECT 
    calendar_date, cohort, SUM(cumulative_pts_severe_icu) cumulative_pts_severe_icu 
FROM (
        SELECT DISTINCT 
            d.calendar_date, d.cohort, a.icu_date, COUNT(*) cumulative_pts_severe_icu
        FROM 
            FOURCE_LOCALDAILYCOUNTS_PED d
        JOIN fource_LocalPatientSummary_PED a
        ON  a.cohort=d.cohort  and a.icu_date>to_date('01-JAN-1900', 'dd-mon-yyyy') and a.icu_date<=d.calendar_date and a.severe = 1
        GROUP BY 
            d.cohort,d.calendar_date, a.icu_date 
        )
GROUP BY calendar_date, cohort)x ON (x.cohort = c.cohort and x.calendar_date=c.calendar_date)
when matched then
update set c.cumulative_pts_severe_icu = x.cumulative_pts_severe_icu;
...
--------------------------------------------------------------------------------
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALCLINICALCOURSE_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LOCALCLINICALCOURSE_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	days_since_admission number(10) not null,
	pts_all_in_hosp number(10) not null,
	pts_all_in_icu number(10) not null,
	pts_all_dead number(10) not null,
	pts_severe_by_this_day number(10) not null,
	pts_ever_severe_in_hosp number(10) not null,
	pts_ever_severe_in_icu number(10) not null,
	pts_ever_severe_dead number(10) not null
);
ALTER TABLE I2B2REPORTING.FOURCE_LOCALCLINICALCOURSE_PED add primary key (cohort, days_since_admission, siteid);
INSERT INTO I2B2REPORTING.FOURCE_LOCALCLINICALCOURSE_PED
	select  (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) siteid,
	...
	from I2B2REPORTING.FOURCE_CONFIG_PED x
		cross join I2B2REPORTING.FOURCE_LOCALPATIENTCLINICALCOURSE_PED c  -- <-- per line 3385 in Michele Morris' code, should this be FOURCE_LOCALPATIENTCLINICALCOURSE?
		inner join I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED p
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALAGESEX_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LOCALAGESEX_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	age_group varchar(20) not null,
	mean_age number(18,10) not null,
	sex varchar(10) not null,
	pts_all number(10) not null,
	pts_ever_severe number(10) not null
);
ALTER TABLE I2B2REPORTING.FOURCE_LOCALAGESEX_PED add primary key (cohort, age_group, sex, siteid);
INSERT INTO I2B2REPORTING.FOURCE_LOCALAGESEX_PED
	select  (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) siteid, cohort, age_group, nvl(avg(cast(nullif(age,-999) as numeric(18,10))),-999), sex, count(*), sum(severe)
		from I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY_PED
		group by cohort, age_group, sex
	union all
	select  (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) siteid, cohort, 'all', nvl(avg(cast(nullif(age,-999) as numeric(18,10))),-999), sex, count(*), sum(severe)
		from I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY_PED
		group by cohort, sex
	union all
	select  (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) siteid, cohort, age_group, nvl(avg(cast(nullif(age,-999) as numeric(18,10))),-999), 'all', count(*), sum(severe)
		from I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY_PED
		group by cohort, age_group
	union all
	select  (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) siteid, cohort, 'all', nvl(avg(cast(nullif(age,-999) as numeric(18,10))),-999), 'all', count(*), sum(severe)
		from I2B2REPORTING.FOURCE_LOCALPATIENTSUMMARY_PED
		group by cohort;
COMMIT;
...
-- LocalLabs
--------------------------------------------------------------------------------
--drop table fource_LocalLabs_PED;
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALLABS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LOCALLABS_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	loinc varchar(20) not null,
	days_since_admission number(10) not null,
	pts_all number(10),
	mean_value_all number(18,5),
	stdev_value_all number(18,5),
	mean_log_value_all number(18,10),
	stdev_log_value_all number(18,10),
	pts_ever_severe number(10),
	mean_value_ever_severe number(18,5),
	stdev_value_ever_severe number(18,5),
	mean_log_value_ever_severe number(18,10),
	stdev_log_value_ever_severe number(18,10),
	pts_never_severe number(10),
	mean_value_never_severe number(18,5),
	stdev_value_never_severe number(18,5),
	mean_log_value_never_severe number(18,10),
	stdev_log_value_never_severe number(18,10)
);
ALTER TABLE I2B2REPORTING.FOURCE_LOCALLABS_PED add primary key (cohort, loinc, days_since_admission, siteid);

INSERT INTO I2B2REPORTING.FOURCE_LOCALLABS_PED
	select  (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) siteid, cohort, concept_code, days_since_admission,
		count(*),
		avg(value),
		nvl(stddev(value),0),
		avg(logvalue),
...
-- LocalDiagProcMed
--------------------------------------------------------------------------------
--drop table fource_LocalDiagProcMed_PED;
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALDIAGPROCMED_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

create table I2B2REPORTING.FOURCE_LOCALDIAGPROCMED_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	concept_type varchar(50) not null,
	concept_code varchar(50) not null,
	pts_all_before_adm number(10),	-- an observation occurred from day @lookback_days to -15 relative to the admission date
	pts_all_since_adm number(10), -- an observation occurred on day >=0
	pts_all_dayN14toN1 number(10), -- an observation occurred from day -14 to -1
	pts_all_day0to29 number(10), -- an observation occurred from day 0 to 29
	pts_all_day30to89 number(10), -- an observation occurred from day 30 to 89
	pts_all_day30plus number(10), -- an observation occurred on day >=30
	pts_all_day90plus number(10), -- an observation occurred on day >=90
	pts_all_1st_day0to29 number(10), -- the first observation is day 0 to 29 (no observations from day @lookback_days to -1)
	pts_all_1st_day30plus number(10), -- the first observation is day >=30 (no observations from day @lookback_days to 29)
	pts_all_1st_day90plus number(10), -- the first observation is day >=90 (no observations from day @lookback_days to 89)
	pts_ever_severe_before_adm number(10),
	pts_ever_severe_since_adm number(10),
	pts_ever_severe_dayN14toN1 number(10),
	pts_ever_severe_day0to29 number(10),
	pts_ever_severe_day30to89 number(10),
	pts_ever_severe_day30plus number(10),
	pts_ever_severe_day90plus number(10),
	pts_ever_severe_1st_day0to29 number(10),
	pts_ever_severe_1st_day30plus number(10),
	pts_ever_severe_1st_day90plus number(10)
);
ALTER TABLE I2B2REPORTING.FOURCE_LOCALDIAGPROCMED_PED add primary key (cohort, concept_type, concept_code, siteid);
INSERT INTO I2B2REPORTING.FOURCE_LOCALDIAGPROCMED_PED
	select  (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1) siteid,
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALRACEBYLOCALCODE_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LOCALRACEBYLOCALCODE_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	race_local_code varchar(500) not null,
	race_4ce varchar(100) not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_LOCALRACEBYLOCALCODE_PED add primary key (cohort, race_local_code, siteid);
INSERT INTO I2B2REPORTING.FOURCE_LOCALRACEBYLOCALCODE_PED
	select (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1), r.cohort, r.race_local_code, r.race_4ce, count(*), sum(p.severe)
	from I2B2REPORTING.FOURCE_LOCALPATIENTRACE_PED r
		inner join I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED p
...
--------------------------------------------------------------------------------
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LOCALRACEBY4CECODE_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LOCALRACEBY4CECODE_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	race_4ce varchar(100) not null,
	pts_all number(10) not null,
 ...
 ALTER TABLE I2B2REPORTING.FOURCE_LOCALRACEBY4CECODE_PED add primary key (cohort, race_4ce, siteid);
 INSERT INTO I2B2REPORTING.FOURCE_LOCALRACEBY4CECODE_PED
	select (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1), r.cohort, r.race_4ce, count(*), sum(p.severe)
	from I2B2REPORTING.FOURCE_LOCALPATIENTRACE_PED r
		inner join I2B2REPORTING.FOURCE_COHORT_PATIENTS_PED p
			on r.cohort=p.cohort and r.patient_num=p.patient_num
...
-------------------------------------------------------------------------------
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_COHORTS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_COHORTS_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	source_data_updated_date date not null,
	earliest_adm_date date not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_COHORTS_PED add primary key (cohort, siteid);
--truncate table fource_Cohorts;

INSERT INTO I2B2REPORTING.FOURCE_COHORTS_PED
	select (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1), cohort, source_data_updated_date, earliest_adm_date, latest_adm_date
	from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED
...
-- DailyCounts
--------------------------------------------------------------------------------
--drop table fource_DailyCounts_PED;
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_DAILYCOUNTS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

create table I2B2REPORTING.FOURCE_DAILYCOUNTS_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	calendar_date date not null,
	cumulative_pts_all number(10) not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_DAILYCOUNTS_PED add primary key (cohort, calendar_date, siteid);
INSERT INTO I2B2REPORTING.FOURCE_DAILYCOUNTS_PED
	select *
	from I2B2REPORTING.FOURCE_LOCALDAILYCOUNTS_PED
	where cohort in (select cohort from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED where include_in_phase1=1);
COMMIT;

--------------------------------------------------------------------------------
-- ClinicalCourse
--------------------------------------------------------------------------------
--drop table fource_CLinicalCourse_PED;
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_CLINICALCOURSE_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_CLINICALCOURSE_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	days_since_admission number(10) not null,
	pts_all_in_hosp number(10) not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_CLINICALCOURSE_PED add primary key (cohort, days_since_admission);
INSERT INTO I2B2REPORTING.FOURCE_CLINICALCOURSE_PED
	select *
	from I2B2REPORTING.FOURCE_LOCALCLINICALCOURSE_PED
	where cohort in (select cohort from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED where include_in_phase1=1);
COMMIT;
...
-- AgeSex
--------------------------------------------------------------------------------
--drop table fource_AgeSex_PED;
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_AGESEX_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_AGESEX_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	age_group varchar(20) not null,
	mean_age number(18,10) not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_AGESEX_PED add primary key (cohort, age_group, sex, siteid);
INSERT INTO I2B2REPORTING.FOURCE_AGESEX_PED
	select *
	from I2B2REPORTING.FOURCE_LOCALAGESEX_PED
	where cohort in (select cohort from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED where include_in_phase1=1);
COMMIT;
...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LABS_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LABS_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	loinc varchar(20) not null,
	days_since_admission number(10) not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_LABS_PED add primary key (cohort, loinc, days_since_admission, siteid);
INSERT INTO I2B2REPORTING.FOURCE_LABS_PED
	select *
	from I2B2REPORTING.FOURCE_LOCALLABS_PED
	where cohort in (select cohort from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED where include_in_phase1=1);
COMMIT;
...
-- DiagProcMed
--------------------------------------------------------------------------------
--drop table fource_DiagProcMed_PED;
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_DIAGPROCMED_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_DIAGPROCMED_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	concept_type varchar(50) not null,
	concept_code varchar(50) not null,
	pts_all_before_adm number(10),
	pts_all_since_adm number(10),
	pts_all_dayN14toN1 number(10),
	pts_all_day0to29 number(10),
	pts_all_day30to89 number(10),
	pts_all_day30plus number(10),
	pts_all_day90plus number(10),
	pts_all_1st_day0to29 number(10),
	pts_all_1st_day30plus number(10),
	pts_all_1st_day90plus number(10),
	pts_ever_severe_before_adm number(10),
	pts_ever_severe_since_adm number(10),
	pts_ever_severe_dayN14toN1 number(10),
	pts_ever_severe_day0to29 number(10),
...
ALTER TABLE I2B2REPORTING.FOURCE_DIAGPROCMED_PED add primary key (cohort, concept_type, concept_code, siteid);
INSERT INTO I2B2REPORTING.FOURCE_DIAGPROCMED_PED
	select *
	from I2B2REPORTING.FOURCE_LOCALDIAGPROCMED_PED
	where cohort in (select cohort from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED where include_in_phase1=1);
COMMIT;
...
--------------------------------------------------------------------------------
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_RACEBYLOCALCODE_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_RACEBYLOCALCODE_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	race_local_code varchar(500) not null,
	race_4ce varchar(100) not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_RACEBYLOCALCODE_PED add primary key (cohort, race_local_code, siteid);
INSERT INTO I2B2REPORTING.FOURCE_RACEBYLOCALCODE_PED
	select *
	from I2B2REPORTING.FOURCE_LOCALRACEBYLOCALCODE_PED
	where cohort in (select cohort from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED where include_in_phase1=1);
COMMIT;
...
-- RaceBy4CECode
--------------------------------------------------------------------------------
--drop table fource_RaceBy4CECode_PED;
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_RACEBY4CECODE_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_RACEBY4CECODE_PED (
	siteid varchar(50) not null,
	cohort varchar(50) not null,
	race_4ce varchar(100) not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_RACEBY4CECODE_PED add primary key (cohort, race_4ce, siteid);
INSERT INTO I2B2REPORTING.FOURCE_RACEBY4CECODE_PED
	select *
	from I2B2REPORTING.FOURCE_LOCALRACEBY4CECODE_PED
	where cohort in (select cohort from I2B2REPORTING.FOURCE_COHORT_CONFIG_PED where include_in_phase1=1);
COMMIT;
...
--------------------------------------------------------------------------------
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE I2B2REPORTING.FOURCE_LABCODES_PED CASCADE CONSTRAINTS PURGE';
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE I2B2REPORTING.FOURCE_LABCODES_PED (
	siteid varchar(50) not null,
	fource_loinc varchar(20) not null,
	fource_lab_units varchar(20) not null,
	fource_lab_name varchar(100) not null,
...
ALTER TABLE I2B2REPORTING.FOURCE_LABCODES_PED add primary key (fource_loinc, local_lab_code, local_lab_units, siteid);
INSERT INTO I2B2REPORTING.FOURCE_LABCODES_PED
	select (select siteid from I2B2REPORTING.FOURCE_CONFIG_PED where rownum = 1), fource_loinc, fource_lab_units, fource_lab_name, scale_factor, replace(local_lab_code,',',';'), replace(local_lab_units,',',';'), replace(local_lab_name,',',';'), replace(notes,',',';')
	from I2B2REPORTING.FOURCE_LAB_MAP_REPORT_PED;
COMMIT;


-- ####  sem -  NO OBFUSCATION OR BLURRING or removal of small counts is done for U-M  ######
/*
--******************************************************************************
--******************************************************************************
...
--------------------------------------------------------------------------------
    update fource_ped_DailyCounts
        set cumulative_pts_all = (round(cumulative_pts_all/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			cumulative_pts_icu = (round(cumulative_pts_icu/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			cumulative_pts_dead = (round(cumulative_pts_dead/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			cumulative_pts_severe = (round(cumulative_pts_severe/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			cumulative_pts_severe_icu = (round(cumulative_pts_severe_icu/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			cumulative_pts_severe_dead = (round(cumulative_pts_severe_dead/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_in_hosp_on_this_date = (round(pts_in_hosp_on_this_date/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_in_icu_on_this_date = (round(pts_in_icu_on_this_date/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_severe_in_hosp_on_date = (round(pts_severe_in_hosp_on_date/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_severe_in_icu_on_date = (round(pts_severe_in_icu_on_date/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) 
	where (select obfuscation_blur from fource_ped_config where rownum = 1) > 0;
	update fource_ped_ClinicalCourse
		set pts_all_in_hosp = (round(pts_all_in_hosp/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_all_in_icu = (round(pts_all_in_icu/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_all_dead = (round(pts_all_dead/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_severe_by_this_day = (round(pts_severe_by_this_day/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_in_hosp = (round(pts_ever_severe_in_hosp/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_in_icu = (round(pts_ever_severe_in_icu/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_dead = (round(pts_ever_severe_dead/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1)))
	where (select obfuscation_blur from fource_ped_config where rownum = 1) > 0;
	update fource_ped_AgeSex
		set pts_all = (round(pts_all/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe = (round(pts_ever_severe/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1)))
	where (select obfuscation_blur from fource_ped_config where rownum = 1) > 0;
	update fource_ped_Labs
		set pts_all = (round(pts_all/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe = (round(pts_ever_severe/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_never_severe = (round(pts_never_severe/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1)))
    where (select obfuscation_blur from fource_ped_config where rownum = 1) > 0;
	update fource_ped_DiagProcMed
		set pts_all_before_adm = (round(pts_all_before_adm/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_all_since_adm = (round(pts_all_since_adm/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_all_dayN14toN1 = (round(pts_all_dayN14toN1/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_all_day0to29 = (round(pts_all_day0to29/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_all_day30to89 = (round(pts_all_day30to89/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_all_day30plus = (round(pts_all_day30plus/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_all_day90plus = (round(pts_all_day90plus/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_all_1st_day0to29 = (round(pts_all_1st_day0to29/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_all_1st_day30plus = (round(pts_all_1st_day30plus/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_all_1st_day90plus = (round(pts_all_1st_day90plus/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_before_adm = (round(pts_ever_severe_before_adm/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_since_adm = (round(pts_ever_severe_since_adm/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_dayN14toN1 = (round(pts_ever_severe_dayN14toN1/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_day0to29 = (round(pts_ever_severe_day0to29/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_day30to89 = (round(pts_ever_severe_day30to89/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_day30plus = (round(pts_ever_severe_day30plus/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_day90plus = (round(pts_ever_severe_day90plus/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_1st_day0to29 = (round(pts_ever_severe_1st_day0to29/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_1st_day30plus = (round(pts_ever_severe_1st_day30plus/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe_1st_day90plus = (round(pts_ever_severe_1st_day90plus/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1)))
    where (select obfuscation_blur from fource_ped_config where rownum = 1) > 0;
	update fource_ped_RaceByLocalCode
		set pts_all = (round(pts_all/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe = (round(pts_ever_severe/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1)))
	where (select obfuscation_blur from fource_ped_config where rownum = 1) > 0;
	update fource_ped_RaceBy4CECode
		set pts_all = (round(pts_all/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1))) ,
			pts_ever_severe = (round(pts_ever_severe/5.0,0)*5)+greatest(-1*(select obfuscation_blur from fource_ped_config where rownum = 1),least(round(dbms_random.normal*2.8,0),(select obfuscation_blur from fource_ped_config where rownum = 1)))
    where (select obfuscation_blur from fource_ped_config where rownum = 1) > 0;
...
--------------------------------------------------------------------------------
    update fource_ped_DailyCounts
		set cumulative_pts_all = (case when cumulative_pts_all<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else cumulative_pts_all end),
			cumulative_pts_icu = (case when cumulative_pts_icu=-999 then -999 when cumulative_pts_icu<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else cumulative_pts_icu end),
			cumulative_pts_dead = (case when cumulative_pts_dead=-999 then -999 when cumulative_pts_dead<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else cumulative_pts_dead end),
			cumulative_pts_severe = (case when cumulative_pts_severe<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else cumulative_pts_severe end),
			cumulative_pts_severe_icu = (case when cumulative_pts_severe_icu=-999 then -999 when cumulative_pts_severe_icu<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else cumulative_pts_severe_icu end),
			cumulative_pts_severe_dead = (case when cumulative_pts_severe_dead=-999 then -999 when cumulative_pts_severe_dead<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else cumulative_pts_severe_dead end),
			pts_in_hosp_on_this_date = (case when pts_in_hosp_on_this_date<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_in_hosp_on_this_date end),
			pts_in_icu_on_this_date = (case when pts_in_icu_on_this_date=-999 then -999 when pts_in_icu_on_this_date<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_in_icu_on_this_date end),
			pts_severe_in_hosp_on_date = (case when pts_severe_in_hosp_on_date<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_severe_in_hosp_on_date end),
			pts_severe_in_icu_on_date = (case when pts_severe_in_icu_on_date=-999 then -999 when pts_severe_in_icu_on_date<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_severe_in_icu_on_date end)
	where (select obfuscation_small_count_mask from fource_ped_config where rownum=1)  > 0;
    
	update fource_ped_ClinicalCourse
		set pts_all_in_hosp = (case when pts_all_in_hosp<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_in_hosp end),
			pts_all_in_icu = (case when pts_all_in_icu=-999 then -999 when pts_all_in_icu<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_in_icu end),
			pts_all_dead = (case when pts_all_dead=-999 then -999 when pts_all_dead<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_dead end),
			pts_severe_by_this_day = (case when pts_severe_by_this_day<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_severe_by_this_day end),
			pts_ever_severe_in_hosp = (case when pts_ever_severe_in_hosp<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_in_hosp end),
			pts_ever_severe_in_icu = (case when pts_ever_severe_in_icu=-999 then -999 when pts_ever_severe_in_icu<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_in_icu end),
			pts_ever_severe_dead = (case when pts_ever_severe_dead=-999 then -999 when pts_ever_severe_dead<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_dead end)
	where (select obfuscation_small_count_mask from fource_ped_config where rownum=1)  > 0;

	update fource_ped_AgeSex
		set pts_all = (case when pts_all<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all end),
			pts_ever_severe = (case when pts_ever_severe<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe end)
	where (select obfuscation_small_count_mask from fource_ped_config where rownum=1)  > 0;

	update fource_ped_Labs
		set pts_all=-99, mean_value_all=-99, stddev_value_all=-99, mean_log_value_all=-99, stddev_log_value_all=-99
		where pts_all<(select obfuscation_small_count_mask from fource_ped_config where rownum=1)
	and (select obfuscation_small_count_mask from fource_ped_config where rownum=1)  > 0;
    
	update fource_ped_Labs -- Need to mask both ever_severe and never_severe if either of them are below the small count threshold, since all=ever+never
		...
		where (pts_ever_severe<(select obfuscation_small_count_mask from fource_ped_config where rownum=1)) or (pts_never_severe<(select obfuscation_small_count_mask from fource_ped_config where rownum=1))
	and (select obfuscation_small_count_mask from fource_ped_config where rownum=1)  > 0;

	update fource_ped_DiagProcMed
		set pts_all_before_adm = (case when pts_all_before_adm<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_before_adm end),
			pts_all_since_adm = (case when pts_all_since_adm<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_since_adm end),
			pts_all_dayN14toN1 = (case when pts_all_dayN14toN1<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_dayN14toN1 end),
			pts_all_day0to29 = (case when pts_all_day0to29<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_day0to29 end),
			pts_all_day30to89 = (case when pts_all_day30to89<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_day30to89 end),
			pts_all_day30plus = (case when pts_all_day30plus<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_day30plus end),
			pts_all_day90plus = (case when pts_all_day90plus<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_day90plus end),
			pts_all_1st_day0to29 = (case when pts_all_1st_day0to29<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_1st_day0to29 end),
			pts_all_1st_day30plus = (case when pts_all_1st_day30plus<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_1st_day30plus end),
			pts_all_1st_day90plus = (case when pts_all_1st_day90plus<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_all_1st_day90plus end),
			pts_ever_severe_before_adm = (case when pts_ever_severe_before_adm<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_before_adm end),
			pts_ever_severe_since_adm = (case when pts_ever_severe_since_adm<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_since_adm end),
			pts_ever_severe_dayN14toN1 = (case when pts_ever_severe_dayN14toN1<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_dayN14toN1 end),
			pts_ever_severe_day0to29 = (case when pts_ever_severe_day0to29<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_day0to29 end),
			pts_ever_severe_day30to89 = (case when pts_ever_severe_day30to89<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_day30to89 end),
			pts_ever_severe_day30plus = (case when pts_ever_severe_day30plus<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_day30plus end),
			pts_ever_severe_day90plus = (case when pts_ever_severe_day90plus<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_day90plus end),
			pts_ever_severe_1st_day0to29 = (case when pts_ever_severe_1st_day0to29<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_1st_day0to29 end),
			pts_ever_severe_1st_day30plus = (case when pts_ever_severe_1st_day30plus<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_1st_day30plus end),
			pts_ever_severe_1st_day90plus = (case when pts_ever_severe_1st_day90plus<(select obfuscation_small_count_mask from fource_ped_config where rownum=1) then -99 else pts_ever_severe_1st_day90plus end)
	where (select obfuscation_small_count_mask from fource_ped_config where rownum=1)  > 0;
...
--------------------------------------------------------------------------------
	update fource_ped_AgeSex
		...
		where (age_group<>'all') and (sex<>'all') and (select obfuscation_agesex from fource_ped_config where rownum = 1) = 1;

--------------------------------------------------------------------------------
-- Delete small counts.
--------------------------------------------------------------------------------
	delete from fource_ped_DailyCounts where cumulative_pts_all<(select obfuscation_small_count_delete from fource_ped_config where rownum = 1) 
        and (select obfuscation_small_count_delete from fource_ped_config where rownum = 1) > 0;
	delete from fource_ped_ClinicalCourse 
        where pts_all_in_hosp<(select obfuscation_small_count_delete from fource_ped_config where rownum = 1) and pts_all_dead<(select obfuscation_small_count_delete from fource_ped_config where rownum = 1) 
        and pts_severe_by_this_day<(select obfuscation_small_count_delete from fource_ped_config where rownum = 1)
        and (select obfuscation_small_count_delete from fource_ped_config where rownum = 1) > 0;
	delete from fource_ped_Labs where pts_all<(select obfuscation_small_count_delete from fource_ped_config where rownum = 1)
        and (select obfuscation_small_count_delete from fource_ped_config where rownum = 1) > 0;
	delete from fource_ped_DiagProcMed 
		where pts_all_before_adm<(select obfuscation_small_count_delete from fource_ped_config where rownum = 1) 
			and pts_all_since_adm<(select obfuscation_small_count_delete from fource_ped_config where rownum = 1) 
			and pts_all_dayN14toN1<(select obfuscation_small_count_delete from fource_ped_config where rownum = 1)
            and (select obfuscation_small_count_delete from fource_ped_config where rownum = 1) > 0;
	...
	--delete from fource_ped_AgeSex where pts_all<(select obfuscation_small_count_delete from fource_ped_config where rownum = 1)
	--delete from fource_ped_RaceByLocalCode where pts_all<@obfuscation_small_count_delete
	--delete from fource_ped_RaceBy4CECode where pts_all<@obfuscation_small_count_delete

--------------------------------------------------------------------------------
-- Delete cohorts that should not be included in Phase2 patient level files
--------------------------------------------------------------------------------
--Phase 2 patient level tables
delete from fource_ped_LocalPatientClinicalCourse where cohort in (select cohort from fource_ped_cohort_config where include_in_phase2=0);
delete from fource_ped_LocalPatientSummary where cohort in (select cohort from fource_ped_cohort_config where include_in_phase2=0);
delete from fource_ped_LocalPatientObservations where cohort in (select cohort from fource_ped_cohort_config where include_in_phase2=0);
delete from fource_ped_LocalPatientRace where cohort in (select cohort from fource_ped_cohort_config where include_in_phase2=0);

--------------------------------------------------------------------------------
-- Remove rows where all values are zeros to reduce the size of the files
--------------------------------------------------------------------------------
delete from fource_ped_LocalPatientClinicalCourse where in_hospital=0 and severe=0 and in_icu=0 and dead=0;
...
--------------------------------------------------------------------------------
create table fource_ped_LocalPatientMapping (
	siteid varchar(50) not null,
	patient_num int not null,
	study_num int not null
);
alter table fource_ped_LocalPatientMapping add primary key (patient_num, study_num, siteid);

--Create new patient_nums and a mapping table
insert into fource_ped_LocalPatientMapping (siteid, patient_num, study_num)
		select (select siteid from fource_ped_config where   rownum = 1) siteid, 
        patient_num, rownum 
		from (
			select distinct patient_num
			from fource_ped_LocalPatientSummary
		) t
        where (select replace_patient_num from fource_ped_config where  rownum = 1) = 1;

ALTER TABLE fource_ped_LocalPatientClinicalCourse ADD patient_num_orig int;
update fource_ped_LocalPatientClinicalCourse set patient_num_orig = patient_num;               
merge into fource_ped_LocalPatientClinicalCourse t 
using (select patient_num, study_num 
        from fource_ped_LocalPatientMapping where (select replace_patient_num from fource_ped_config where rownum = 1) = 1) m
on (t.patient_num_orig = m.patient_num)
when matched then
update set patient_num = m.study_num;
ALTER TABLE fource_ped_LocalPatientClinicalCourse drop column patient_num_orig;
commit;

ALTER TABLE fource_ped_LocalPatientSummary ADD patient_num_orig int;
update fource_ped_LocalPatientSummary set patient_num_orig = patient_num;               
merge into fource_ped_LocalPatientSummary t 
using (select patient_num, study_num 
        from fource_ped_LocalPatientMapping where (select replace_patient_num from fource_ped_config where rownum = 1) = 1) m
on (t.patient_num_orig = m.patient_num)
when matched then
update set patient_num = m.study_num;
ALTER TABLE fource_ped_LocalPatientSummary drop column patient_num_orig;
commit;

ALTER TABLE fource_ped_LocalPatientObservations ADD patient_num_orig int;
update fource_ped_LocalPatientObservations set patient_num_orig = patient_num;               
merge into fource_ped_LocalPatientObservations t 
using (select patient_num, study_num 
        from fource_ped_LocalPatientMapping where (select replace_patient_num from fource_ped_config where rownum = 1) = 1) m
on (t.patient_num_orig = m.patient_num)
when matched then
update set patient_num = m.study_num;
ALTER TABLE fource_ped_LocalPatientObservations drop column patient_num_orig;
commit;

ALTER TABLE fource_ped_LocalPatientRace ADD patient_num_orig int;
update fource_ped_LocalPatientRace set patient_num_orig = patient_num;               
merge into fource_ped_LocalPatientRace t 
using (select patient_num, study_num 
        from fource_ped_LocalPatientMapping where (select replace_patient_num from fource_ped_config where rownum = 1) = 1) m
on (t.patient_num_orig = m.patient_num)
when matched then
update set patient_num = m.study_num;
ALTER TABLE fource_ped_LocalPatientRace drop column patient_num_orig;
commit;

-- Else map existing patient_num to itself
insert into fource_ped_LocalPatientMapping (siteid, patient_num, study_num)
		select distinct  (select siteid from fource_ped_config where   rownum = 1) siteid, patient_num, patient_num
		from fource_ped_LocalPatientSummary
    where (select replace_patient_num from fource_ped_config where   rownum = 1) = 0;
commit;
...
--------------------------------------------------------------------------------
--Phase 2 patient level tables
update fource_ped_LocalPatientClinicalCourse set siteid = (select siteid from fource_ped_config)
update fource_ped_LocalPatientSummary set siteid = (select siteid from fource_ped_config)
update fource_ped_LocalPatientObservations set siteid = (select siteid from fource_ped_config)
update fource_ped_LocalPatientRace set siteid = (select siteid from fource_ped_config)
update fource_ped_LocalPatientMapping set siteid = (select siteid from fource_ped_config)
--Phase 2 aggregate count tables
update fource_ped_LocalCohorts set siteid = (select siteid from fource_ped_config)
update fource_ped_LocalDailyCounts set siteid = (select siteid from fource_ped_config)
update fource_ped_LocalClinicalCourse set siteid = (select siteid from fource_ped_config)
update fource_ped_LocalAgeSex set siteid = (select siteid from fource_ped_config)
update fource_ped_LocalLabs set siteid = (select siteid from fource_ped_config)
update fource_ped_LocalDiagProcMed set siteid = (select siteid from fource_ped_config)
update fource_ped_LocalRaceByLocalCode set siteid = (select siteid from fource_ped_config)
update fource_ped_LocalRaceBy4CECode set siteid = (select siteid from fource_ped_config)
--Phase 1 aggregate count tables
update fource_ped_Cohorts set siteid = (select siteid from fource_ped_config)
update fource_ped_DailyCounts set siteid = (select siteid from fource_ped_config)
update fource_ped_ClinicalCourse set siteid = (select siteid from fource_ped_config)
update fource_ped_AgeSex set siteid = (select siteid from fource_ped_config)
update fource_ped_Labs set siteid = (select siteid from fource_ped_config)
update fource_ped_DiagProcMed set siteid = (select siteid from fource_ped_config)
update fource_ped_RaceByLocalCode set siteid = (select siteid from fource_ped_config)
update fource_ped_RaceBy4CECode set siteid = (select siteid from fource_ped_config)
update fource_ped_LabCodes set siteid = (select siteid from fource_ped_config)
*/
...
/*
	...
	select * from fource_ped_DailyCounts where (select output_phase1_as_columns from fource_ped_config where rownum=1) = 1 order by cohort, calendar_date ;
	select * from fource_ped_ClinicalCourse where (select output_phase1_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, days_since_admission;
	select * from fource_ped_AgeSex where (select output_phase1_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, age_group, sex;
	select * from fource_ped_Labs where (select output_phase1_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, loinc, days_since_admission;
	select * from fource_ped_DiagProcMed where (select output_phase1_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, concept_type, concept_code;
	select * from fource_ped_RaceByLocalCode where (select output_phase1_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, race_local_code;
	select * from fource_ped_RaceBy4CECode where (select output_phase1_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, race_4ce;
	select * from fource_ped_LabCodes where (select output_phase1_as_columns from fource_ped_config where rownum=1) = 1 order by  fource_ped_loinc, local_lab_code, local_lab_units;

	--Phase 2 non-obfuscated local aggregate files
	select * from fource_ped_LocalDailyCounts where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, calendar_date;
	select * from fource_ped_LocalClinicalCourse where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, days_since_admission;
	select * from fource_ped_LocalAgeSex where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, age_group, sex;
	select * from fource_ped_LocalLabs where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, loinc, days_since_admission;
	select * from fource_ped_LocalDiagProcMed where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, concept_type, concept_code;
	select * from fource_ped_LocalRaceByLocalCode where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, race_local_code;
	select * from fource_ped_LocalRaceBy4CECode where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, race_4ce;
	--Phase 2 patient-level files
	select * from fource_ped_LocalPatientClinicalCourse where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, patient_num, days_since_admission;
	select * from fource_ped_LocalPatientSummary where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, patient_num;
	select * from fource_ped_LocalPatientObservations where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, patient_num, days_since_admission, concept_type, concept_code;
	select * from fource_ped_LocalPatientRace where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  cohort, patient_num, race_local_code;
	select * from fource_ped_LocalPatientMapping where (select output_phase2_as_columns from fource_ped_config where rownum=1) = 1 order by  patient_num;
*/

-- EXECUTE DBMS_STATS.GATHER_SCHEMA_STATS(ownname => 'I2B2REPORTING');
